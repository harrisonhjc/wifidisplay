...
status_t ANetworkSession::Session::readMore() {
        ...
        do {
            ...
            if (n < 0) {
                ...
                */
                ...
                notify->post();
                //ALOGI("readMore:reason:kWhatDatagram");
            }
        } while (err == OK);
    ...
    if (!mIsRTSPConnection) {
        ...
        while (mInBuffer.size() >= 2) {
            ...
            notify->post();
            //ALOGI("readMore:reason:kWhatDatagram. !mIsRTSPConnection");
        ...
        for (;;) {
            ...
            if (mInBuffer.size() > 0 && mInBuffer.c_str()[0] == '$') {
                ...
                notify->post();
                //ALOGI("readMore:reason:kWhatBinaryData");
            ...
            notify->post();
            //ALOGI("readMore:reason:kWhatData");
#if 1
...
status_t ANetworkSession::Session::writeMore() {
    if (mState == DATAGRAM) {
        ...
        if (err != OK) {
            notifyError(true , err, "Send datagram failed.");
...
status_t ANetworkSession::createClientOrServer(
    ...
    */
        ...
        in_addr_t x = ntohl(addr.sin_addr.s_addr);
        /*
        ALOGI("ANetworkSession::createClientOrServer:bind:ip:%d.%d.%d.%d:%d",
              (x >> 24),
              (x >> 16) & 0xff,
              (x >> 8) & 0xff,
              x & 0xff,
        ...
        */
        ...
        if (res == 0) {
            if (mode == kModeCreateRTSPServer
                ...
                //ALOGI("ANetworkSession::createClientOrServer:connect:ip=%s,port=%d",
...
status_t ANetworkSession::connectUDPSession(
    ...
    //ALOGI("connectUDPSession:ip=%s,port=%d.",remoteHost,remotePort);
