...
struct WifiDisplaySink : public AHandler {
    ...
    void start(const char *sourceHost, int32_t sourcePort,
               ANativeWindow* window, const char*, JavaVM *jvm);
    void start(const char *uri);
    void stop();

    static void cb_hdcpAuthComplete(void* vself, int data);
    void hdcpAuthComplete(int data);
...
private:
    ...
    int32_t mSessionPlayId;
    int32_t msessionSetupId;
    int32_t mCseq;
    ...
    enum {
        ...
        kWhatWfdSetScale,
        kWhatWfdTouchEvent,
        kWhatVideoResolution,
        kWhatWatchDog,
        kWhatWfdSetAudioPath,
        kWhatWfdSetVolume,
        kWhatWfdKeyEvent,
    ...
    struct timeval liveTimeStart;
    struct timeval liveTimeEnd;
    int mPortUIBC;
    bool firstShowup;
